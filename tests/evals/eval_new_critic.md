...
**Question:** In Spark Architecture, what are the four main components and what is the role of each?
...
2. **Cluster Manager** – Acts like an HR department; manages resources (CPU, memory) across the cluster. Options include YARN, Kubernetes, Mesos, or Standalone.
3. **Executors** – The workers; each runs on a separate machine and processes a chunk of data, doing the actual computation.
...
**Question:** When an iron nail is placed in a copper sulphate (CuSO₄) solution, a displacement reaction occurs. What is the chemical equation for this reaction, what colour change is observed, and why does this reaction occur?

**Answer:** The chemical equation is: Fe + CuSO₄ → FeSO₄ + Cu. The blue colour of the copper sulphate solution fades and a green colour appears (due to the formation of iron sulphate, FeSO₄). This reaction occurs because iron is more reactive than copper, so iron displaces copper from the copper sulphate solution.
...
**Question:** For a first order reaction, the half-life is given by the formula t₁/₂ = ___.
...
**Question:** The number of reacting species (atoms, ions or molecules) taking part in an elementary reaction and bringing about the chemical change is called the ___ of a reaction.
...
**Question:** In PySpark, RDD stands for ___ Distributed Dataset.
...
**Answer:** **(a) Average rate of decomposition of H₂O₂:**

Using the stoichiometric coefficient of 2 for H₂O₂:
...
From stoichiometry: 2H₂O₂ → 2H₂O + O₂
The molar ratio of O₂ to H₂O₂ is 1:2, so the rate of O₂ production is half the rate of H₂O₂ disappearance:

Rate of O₂ production = (1/2) × 1.00 × 10⁻³
= **5.00 × 10⁻⁴ mol/L/min**

**(c) Converting to mol/L/h:**

1.00 × 10⁻³ mol/L/min × 60 min/h
= **6.00 × 10⁻² mol/L/h**
...
**Question:** A first order decomposition reaction of H2O2 in aqueous solution is given below:

H2O2(aq) → H2O(l) + 1/2 O2(g)

The following concentration data was recorded at 300 K:
...
**Answer:** Using the first order rate constant formula:
...
Step 1 — Compute the concentration ratio:
...
The unit of k is min⁻¹, which is consistent with a first order reaction (units of time⁻¹).
...
**(a)** Calculate the half-life (t₁/₂) of this isotope.

**(b)** How long will it take for 99.9% of the isotope to decay?

Use the first-order half-life formula: **t₁/₂ = 0.693 / k**, and the integrated rate law: **k = (2.303 / t) × log([R]₀ / [R])**
...
When 99.9% has decayed: [R] = [R]₀ − 0.999[R]₀ = 0.001[R]₀

Using the integrated rate law:

k = (2.303 / t) × log([R]₀ / 0.001[R]₀)

k = (2.303 / t) × log(1000)

k = (2.303 / t) × 3 = 6.909 / t

So: t = 6.909 / k = 6.909 / (3.2 × 10⁻⁶ s⁻¹)

t = **2.159 × 10⁶ s**

**Verification:** t / t₁/₂ = 2.159 × 10⁶ / 2.166 × 10⁵ ≈ **10** ✓

This confirms that the time for 99.9% completion is exactly 10 times the half-life for any first-order reaction.
...
where R = 8.314 J mol⁻¹ K⁻¹, calculate the activation energy Eₐ in kJ/mol. Then, using k₁ = 0.045 s⁻¹ at T₁ = 600 K, determine the pre-exponential factor A.

**Answer:** Step 1 — Calculate the ratio and its logarithm:
k₂/k₁ = 0.180 / 0.045 = 4.0
log(4.0) = 0.6021
...
0.6021 = Eₐ × 4.1667 × 10⁻⁴ / (2.303 × 8.314)
2.303 × 8.314 = 19.147 J mol⁻¹ K⁻¹
0.6021 = Eₐ × 4.1667 × 10⁻⁴ / 19.147
Eₐ = 0.6021 × 19.147 / (4.1667 × 10⁻⁴)
Eₐ = 11.529 / 4.1667 × 10⁻⁴
Eₐ ≈ 27,670 J/mol ≈ 27.67 kJ/mol

Step 4 — Find the pre-exponential factor A using k = A·e^(−Eₐ/RT) at T₁ = 600 K:
Eₐ/RT₁ = 27,670 / (8.314 × 600) = 27,670 / 4,988.4 = 5.547
e^(−5.547) = 0.003876
0.045 = A × 0.003876
A = 0.045 / 0.003876 ≈ 11.61 s⁻¹
...
**Question:** A student adds MnO₂ to a reaction vessel containing KClO₃ and heats the mixture. The decomposition reaction proceeds much faster than without MnO₂. After the reaction is complete, the student recovers the same mass of MnO₂ that was added initially. What role did MnO₂ play, and how does the intermediate complex theory explain its behaviour during the reaction?

**Answer:** MnO₂ acted as a catalyst — it increased the rate of decomposition of KClO₃ (2KClO₃ → 2KCl + 3O₂) without undergoing any permanent chemical change, which is why its mass was recovered unchanged. According to intermediate complex theory, MnO₂ formed temporary bonds with the KClO₃ reactant molecules, creating an intermediate complex. This complex then decomposed to yield the products (KCl and O₂) and regenerated MnO₂. In doing so, the catalyst provided an alternate reaction pathway with a lower activation energy, allowing the reaction to proceed faster.
...
What happens if you stop here and never call an action like `grouped.show()`? When does actual data processing begin, and why?

**Answer:** Nothing is executed. PySpark's lazy evaluation means that `read.csv()`, `filter()`, and `groupBy().count()` are all either a read or transformations — they only add steps to the DAG (Directed Acyclic Graph) plan but do NOT process any data. Actual execution begins only when an **action** (such as `show()`, `count()`, `collect()`, or `write()`) is called. This design lets Spark optimize the full query plan — applying techniques like predicate pushdown and column pruning — before any data is read from disk.
...
**Question:** A steel bridge is coated with zinc (galvanisation) to prevent rusting. Over time, a small section of the zinc coating gets scratched off, exposing the iron underneath. What happens to the iron at the scratched spot, and why does galvanisation still protect it even after the scratch?

**Answer:** Even after the zinc coating is scratched and iron is exposed, the iron does NOT rust at that spot. This is because zinc is more reactive than iron, so zinc preferentially reacts with oxygen and moisture instead of iron. Zinc acts as a sacrificial anode — it corrodes in place of the iron, continuing to protect the underlying steel. This is in contrast to painting, where a scratch would directly expose iron to the environment and allow rusting to begin.
...
What happens as a result, and why is this considered a critical mistake?

**Answer:** Using `coalesce(1)` forces all 500 GB of data onto a single partition on one machine. This defeats the purpose of distributed computing because Spark can no longer spread the workload across multiple executors. The single machine must hold the entire dataset in memory, which will very likely cause an **OutOfMemoryError**. The source text explicitly warns: *"Never use coalesce(1) on large datasets as it forces all data onto one partition (one machine), defeating the purpose of distributed computing and likely causing an OutOfMemoryError."*
