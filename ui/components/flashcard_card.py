...
def render_flashcard_list(sub_id, status="approved"):
    fcs = api_client.get_flashcards_by_subtopic(sub_id, status)
    if not fcs:
    ...
    for fc in fcs:
        if status == "approved":
            src = _resolve_source(fc.get("chunk_id"))
            ...
            render_flashcard_review_card(fc, status)


def render_flashcard_review_card(fc: dict, current_status: str):
    ...
    # Fetch source once; reused for badge and snippet below.
